        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {uniqueCategories.map((category, index) => (
            <motion.div
              ...
              className="group"
            >
              <div className="relative rounded-2xl p-8 h-full transition-all duration-500 overflow-hidden
                bg-[linear-gradient(to_bottom,_#170A24_0%,_#150920_50%,_#12071A_100%)]
                border border-primary/20 hover:border-primary/40 group-hover:transform group-hover:scale-105">
                
                {/* Background glow on hover */}
                <motion.div 
                  className="absolute inset-0 bg-gradient-to-r from-primary/10 to-transparent rounded-2xl opacity-0 group-hover:opacity-100"
                  transition={{ duration: 0.3 }}
                />

                <div className="relative z-10">
                  <h3 className="text-xl font-bold text-white mb-6 group-hover:text-primary transition-colors duration-300">
                    {category}
                  </h3>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {technologies
                      ...
                      .map((tech, techIndex) => (
                        <motion.span 
                          key={techIndex}
                          className="bg-white/10 text-white/80 rounded-full px-3 py-1 text-sm border border-white/20 
                            hover:bg-primary/20 hover:border-primary/30 hover:text-white transition-all duration-300"
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                        >
                          {tech.name}
